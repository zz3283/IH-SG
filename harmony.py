...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='config/harmony.json',  ##train:/shadow_generation.json
                        help='JSON file for configuration')
    parser.add_argument('-p', '--phase', default='train',type=str, choices=['train', 'val'], ##train  or val
                        help='Run either train(training) or val(generation)', )
    parser.add_argument('-gpu', '--gpu_ids', default='3,4,5',type=str)
    ...
    if opt['phase'] == 'train':
        while current_step < n_iter:
            ...
            for _, train_data in enumerate(train_loader):
                ...
                if current_step % opt['train']['val_freq'] == 0:
                    ...
                    for _,  val_data in enumerate(val_loader):
                        ...
                        Mask_img = Metrics.tensor2img(visuals['M'], min_max=(0, 1))  #
            ...
            print("code run total time：%.8s s" % timespan)
        ...
        for _,  val_data in enumerate(val_loader):
            ...
            if input_img_mode == 'single':
                ...
                Metrics.save_img(
                    result_img_grid, '{}/{}_{}_result_process.png'.format(result_path, current_step, idx))
                Metrics.save_img(
                    Metrics.tensor2img(visuals['Result'][-1], min_max=(0,1)), '{}/{}_{}_result.png'.format(result_path, current_step, idx))
                Metrics.save_img(
                    Mask_img, '{}/{}_{}_mask.png'.format(result_path, current_step, idx))

                # zt_img_grid = Metrics.tensor2img(visuals['Zt'])  # uint8
                # Metrics.save_img(
                #     zt_img_grid, '{}/{}_{}_zt_process.png'.format(result_path, current_step, idx))
                # Metrics.save_img(
                #     Metrics.tensor2img(visuals['Zt'][-1]), '{}/{}_{}_zt.png'.format(result_path, current_step, idx))
