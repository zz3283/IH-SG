...
class testdata(Dataset):
    def __init__(self, dataroot, datatype, split='test', data_len=-1):
        self.datatype = datatype
        self.data_len = data_len
        self.split = split
        self.image_paths=dataroot
        self.mask_path, self.input_path,self.ref = [], [], []
        """
        合成图像：IMG_2230_2.jpg 
        gt：IMG_2230.jpg
        ...
        if self.split=='val':
            # for shadow
            # # self.gt_path = Util.get_paths_from_images(os.path.join(dataroot, 'shadow'))
            # self.input_path = Util.get_paths_from_images(os.path.join(dataroot, 'comp'))
            # self.mask_path = Util.get_paths_from_images(os.path.join(dataroot, 'mask'))
            # #--验证图像重光照
            # self.ref_path=Util.get_paths_from_images(os.path.join(dataroot, 'ref'))
    ...
    def __getitem__(self, index):
        ...
        [ img_input,img_ref] = Util.transform_augment(
            [img_input,img_ref], split=self.split, min_max=(0, 1))
        [img_mask] = Util.transform_augment([img_mask], split=self.split, min_max=(0, 1))
        return {'Input': img_input,'Mask': img_mask,'Index': index,'ref':img_ref}
