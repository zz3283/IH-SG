...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='config/har_test_2.json',
                        help='JSON file for configuration')
    parser.add_argument('-p', '--phase', type=str, choices=['val'], help='val(generation)', default='val')
    parser.add_argument('-gpu', '--gpu_ids', type=str, default='7,6,5,4,3')
    ...
    for _,  test_data in enumerate(test_loader):
        ...
        if input_img_mode == 'single':
            ...
            for iter in range(0, sample_num):
                Metrics.save_img(
                    Metrics.tensor2img(input_img[iter],min_max=(0,1)), '{}/{}_{}_input_{}.png'.format(result_path, current_step, idx, iter))
        else:
            # grid img
            input_img = Metrics.tensor2img(visuals['Input'],min_max=(0,1))  # uint8
            Metrics.save_img(
            ...
            Metrics.save_img(
                Metrics.tensor2img(visuals['Input'][-1],min_max=(0,1)), '{}/{}_{}_input.png'.format(result_path, current_step, idx))
