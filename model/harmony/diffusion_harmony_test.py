...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, condition_x=None):
        ...
        if condition_x is not None:
            x_start,mask= self.denoise_fn(torch.cat([x, condition_x], dim=1), noise_level)
    ...
    def p_sample_loop(self, x_in, continous=False):
        ...
        if self.conditional:
            ...
            # vt_img = vt
            for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
                mean,var,x_start,mask= self.p_sample(zt, i, condition_x=x)
                if i > 0:
                    noise = torch.randn_like(x)
                else:
                    noise = 0
                zt = mean + noise * (0.5 * var).exp()  ####
                zt.clamp_(0., 1.)

                # if i % sample_inter == 0:
                #     ret_img = torch.cat([ret_img, zt], dim=0)      # concat img in the batch dim
                #     zt_img = torch.cat([zt_img, zt], dim=0)
                if i % sample_inter == 0:
                    zt_img = torch.cat([zt_img, zt], dim=0)

        if continous:
            return [x_start, zt_img ,mask]
        else:
            """post deal"""
            # ret_img=ret_img[-1]*mask+ (1 - mask) *x_in['Input']
            # print(ret_img[-1].shape)
            return [x_start[-1], zt_img[-1],mask[-1]]
    ...
    def p_losses(self, x_in, noise=None):
        ...
        noise: torch.Size([2, 3, 256, 256])
        x_recon: torch.Size([2, 3, 256, 256])
        """
