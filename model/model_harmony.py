...
class DDPM(BaseModel):
    ...
    def get_current_visuals(self, sample=False):
        ...
        if sample:
            ...
            out_dict['Result'] = self.Result[0].detach().float().cpu() #x_start zt_img mask
            out_dict['Zt'] = self.Result[1].detach().float().cpu()  # x_start zt_img mask
            out_dict['M'] = self.Result[2].detach().float().cpu()
        ...
        return out_dict
        #  result: [ret_img, zt_img, vt_img]--result,zt, shadow_mask
    ...
    def load_network(self):
        ...
        if load_path is not None:
            ...
            #     gen_path), strict=(not self.opt['model']['finetune_norm']))  ##train

            network.load_state_dict(torch.load(
                gen_path), strict=False)   ##val
            if self.opt['phase'] == 'train':
                # optimizer
