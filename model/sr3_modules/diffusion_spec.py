...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, mask, clip_denoised: bool, condition_x=None):
        batch_size = x.shape[0]
        noise_level = torch.FloatTensor(
            [self.sqrt_alphas_cumprod_prev[t+1]]).repeat(batch_size, 1).to(x.device)
        if condition_x is not None:
            et_1 = self.denoise_fn(torch.cat([x, condition_x, mask], dim=1), noise_level)
            ...
            et_1 = self.denoise_fn(x, noise_level)
    ...
    @torch.no_grad()
    def p_sample(self, x, t, mask, clip_denoised=True, condition_x=None):
        xt_1 = self.p_mean_variance(
            x=x, t=t, mask=mask, clip_denoised=clip_denoised, condition_x=condition_x)
        return xt_1  # xt-1, mask is the mask_t-1
    ...
    def p_sample_loop(self, x_in, continous=False):
        ...
        if not self.conditional:        # 先不考虑
            ...
            # vt_img = vt
            for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
                xt_1 = self.p_sample(zt, i, mask, condition_x=x)
                # zt = xt_1
                zt = (h * x_in['Input'] + 0.5 * xt_1) / (h * h + 0.5)

                # use xt-1, mask_t-1 to update zt and vt get zt-1 and vt-1
                # penalize parameters set to be 0.5, rou and fai
                # zt = (x_in['Degra'] * x_in['Input'] + 0.5 * xt_1) / (x_in['Degra'] * x_in['Degra'] + 0.5)
                # vt = (0.5 * x_in['Mask'] + 0.5 * mask_t_1) / (0.5 + 0.5)
                # zt.clamp_(0., 1.)
                # vt.clamp_(0., 1.)
                # vt = torch.where(vt>0.1, 1., 0.)        # 只对阴影区域细化，不会影响到其他区域
                if i % sample_inter == 0:
        ...
        #     vt = x_in['Mask']
        #     shape = x.shape
        #     img = torch.randn(shape, device=device)  # zt, 纯噪声
        #     ret_img = x
        #     for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
        #         img, mask_t_1 = self.p_sample(img, i, vt, condition_x=x)  # img is the xt-1,
        #         # returned img->xt-1, mask_t-1
        #         # use xt-1, mask_t-1 to update zt and vt get zt-1 and vt-1
        #         # penalize parameters set to be 0.5, rou and fai
        #
        #         if i % sample_inter == 0:
        #             ret_img = torch.cat([ret_img, img], dim=0)  # concat img in the batch dim
        #
        # if continous:
        #     return ret_img
        # else:
    ...
    def p_losses(self, x_in, noise=None):
        ...
        if not self.conditional:
            ...
            x_recon = self.denoise_fn(  # there should have other input, y, initial mask, and step t.
                torch.cat([x_noisy, x_in['Input'], x_in['Mask']], dim=1), continuous_sqrt_alpha_cumprod)

        loss = self.loss_func(noise, x_recon)       # sr3 loss for x_recon and noise, x_recon is et_1
        return loss
