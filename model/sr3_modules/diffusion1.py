...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, x, t, vt, clip_denoised=True, condition_x=None):
        ...
        condition_x,换成背景看看
        """
        return xt_1, mask_t_1  # xt-1, shadow_mask is the mask_t-1
    ...
    def p_sample_loop(self, x_in, continous=False):
        ...
        if not self.conditional:        # 先不考虑
            ...
            x = x_in['Input']   # x -> y input是 noshadow image ----->用作condition
            # vt = x_in['Mask']   ## 此处mask是去阴影任务中的阴影区域mask
            vt = x_in['shadow_mask']  ##----## 用于阴影生成--ori mask---shadow_mask
            shape = x.shape
            b, c, h, w = x.shape
            ...
            for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
                ...
                # #7
                # zt = (x_in['Input'] + xt_1)*mask_t_1*0.5 + x_in['Input']*(1-mask_t_1)
                # vt = mask_t_1

                ## for harmony
                zt = (x_in['Input'] * (1-mask_t_1) + mask_t_1* xt_1)
                vt = (0.5 * x_in['Mask'] + 0.5 * mask_t_1) / (0.5 + 0.5)
    ...
    def p_losses(self, x_in, noise=None):
        x_start = x_in['GT']  ##shadow image
        [b, c, h, w] = x_start.shape
        t = np.random.randint(1, self.num_timesteps + 1)
        # mask_start = x_in['Mask']
        mask_start = x_in['shadow_mask']  ##----##初始阴影mask
        ...
        loss_shadow=self.loss_func(x_in['GT']*x_mask,x_recon*x_mask) ##
        loss = loss_diff + 0.5*loss_mask+ 0.5*loss_shadow
        return loss
