...
def init_model(cfg):
    ...
    model = HRNetIHModel(BMCONFIGS['improved_dih256'], small=False)

    model.to(cfg.device)
    model.apply(initializer.XavierGluon(rnd_type='gaussian', magnitude=2.0))
    model.backbone.load_pretrained_weights(cfg.IMAGENET_PRETRAINED_MODELS.HRNETV2_W18)
...
def train(model, cfg, model_cfg, start_epoch=0):
    ...
    trainset = ComposeDataset(
        [
            ...
            HDataset(cfg.mydata_PATH, split='train'),  ### --zj
        ],
    ...
    valset = ComposeDataset(
        [
            ...
            HDataset(cfg.mydata_PATH, split='test'),
        ],
