...
class DeconvDecoder(nn.Module):
    ...
    #
    #     if self.image_fusion:
    #         attention_map = torch.sigmoid(3.0 * self.conv_attention(output))             # blending mask
    #         output = attention_map * image + (1.0 - attention_map) * self.to_rgb(output) #image 是合成图像
    #     else:
    ...
    def forward(self, encoder_outputs, image, mask=None):
        ...
        #----默认 self.image_fusion is true--------#------zj----#
        attention_map = torch.sigmoid(3.0 * self.conv_attention(output))             # blending mask
        mid_output = self.to_rgb(output)
        output = attention_map * image + (1.0 - attention_map) * self.to_rgb(output) #image 是合成图像
        return output,attention_map,mid_output
