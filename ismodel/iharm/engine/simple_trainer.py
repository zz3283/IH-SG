...
class SimpleHTrainer(object):
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            output = self.net(images, masks) ###输入
    ...
    def save_visualization(self, splitted_batch_data, outputs, global_step, prefix):
        ...
        image = (self.denormalizator(image).cpu().numpy() * 255).transpose((1, 2, 0))                       #合成图像
        target_image = (self.denormalizator(target_image).cpu().numpy() * 255).transpose((1, 2, 0))         #GT
        object_mask = np.repeat((object_mask.cpu().numpy() * 255)[:, :, np.newaxis], axis=2, repeats=3)     #前景mask
        predicted_image = (self.denormalizator(outputs['images'].detach()[0]).cpu().numpy() * 255).transpose((1, 2, 0)) #和谐图像
        # print("object_mask:", object_mask.shape)
        # print("predicted_image:",predicted_image.shape)
        #-----------------------zj----------------------------#
        attention_map=outputs['attention_map'][0,0]
        attention_map = np.repeat((attention_map.cpu().numpy() * 255)[:, :, np.newaxis], axis=2, repeats=3) # attention_map
        # print("attention_map:",attention_map.shape)
        mid_output = (self.denormalizator(outputs['mid_output'].detach()[0]).cpu().numpy() * 255).transpose((1, 2, 0))  # mid_output
        # print("mid_output:",mid_output.shape)
        attention_map = np.clip(attention_map, 0, 255)
        mid_output = np.clip(mid_output ,0, 255)
        #------------------------------------------------------#
        predicted_image = np.clip(predicted_image, 0, 255)

        viz_image = np.hstack((image, object_mask, target_image, predicted_image,attention_map,mid_output)).astype(np.uint8)
        # viz_image = np.hstack((image, object_mask, target_image, predicted_image)).astype(np.uint8)  ##ori
