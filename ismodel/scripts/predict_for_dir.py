...
def main():
    ...
    for image_name in tqdm(image_names):
        ...
        gtpath = args.gt
        gt_path = osp.join(gtpath, image_name.split('_')[0] + '.jpg')
        gt_image = cv2.imread(gt_path)
        gt_save = str(cfg.RESULTS_PATH) + "/gt/"
        comp_save = str(cfg.RESULTS_PATH) + "/comp/"
        if not os.path.exists(gt_save):
            os.makedirs(gt_save)
        if not os.path.exists(comp_save):
            os.makedirs(comp_save)

        image = cv2.imread(image_path)
        comp_image=image
        if args.original_size:
            cv2.imwrite(os.path.join(gt_save, image_name), gt_image)
            cv2.imwrite(os.path.join(comp_save, image_name), comp_image)
        else:
            comp_image = cv2.resize(comp_image, resize_shape, cv2.INTER_LINEAR)
            gt_image = cv2.resize(gt_image, resize_shape, cv2.INTER_LINEAR)
            cv2.imwrite(os.path.join(gt_save, image_name), gt_image)
            cv2.imwrite(os.path.join(comp_save, image_name), comp_image)
...
def parse_args():
    ...
    parser.add_argument(
        '--gt', type=str,
        help='Path to directory with .jpg for real_image.'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--original-size', action='store_true', default=True,
